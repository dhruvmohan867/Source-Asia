...
import { formatPrice, formatTime, formatDate, calculateDuration, isWithinHours } from '@/lib/utils';
...
export default function BookingsPage() {
  ...
  const handleCancelConfirm = async () => {
    ...
    const supabase = createClient();
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { data } = await (supabase.rpc as any)('cancel_booking', {
      p_booking_id: selectedBookingId,
      p_user_id: userId,
    });

    const result = data as { success: boolean; error?: string; refund_amount?: number };
    ...
    if (result?.success) {
      ...
      toast.success(`Booking cancelled. Refund: ${formatPrice(result.refund_amount ?? 0)}`);
    } else {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'rescheduled': return <Badge variant="info">Rescheduled</Badge>;
      ...
      {bookings.length === 0 ? (
        ...
        <div className="space-y-4">
          {bookings.map((booking) => {
            ...
            const canCancel = booking.status === 'confirmed' && !isWithinHours(flight.departure_time, 2);
              ...
              >
                ...
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <div className="flex items-center gap-3 mb-1">
                      <span className="font-mono text-sm text-accent">{booking.pnr}</span>
                  ...
                  <p className="text-lg font-bold">{formatPrice(booking.total_amount)}</p>
                </div>
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="text-center">
                    ...
                    <p className="text-sm font-medium mt-1">{formatTime(flight.departure_time)}</p>
                  </div>
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-sm font-medium mt-1">{formatTime(flight.arrival_time)}</p>
                  </div>
                </div>
                  ...
                  <div className="border-t border-border pt-3 mb-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {booking.passengers.map((p) => (
                        <span key={p.id} className="text-xs bg-surface-hover px-2 py-1 rounded-lg">
